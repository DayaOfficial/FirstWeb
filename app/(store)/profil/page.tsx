...
import { ChevronRight, User, LayoutDashboard, KeyRound, LogOut, Search, Filter, Camera, Trash2, Package, Loader2 } from 'lucide-react';
  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        // Supabase not configured — show not logged in state
        setNotLoggedIn(true);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="space-y-8 animate-fade-in">
        <nav className="flex items-center gap-2 text-sm text-on-surface-variant">
          ...
          <span className="text-primary font-semibold">Profil</span>
        </nav>
        <div className="text-center py-20 bg-surface-container-lowest rounded-2xl border border-outline-variant/30">
          <User size={56} className="mx-auto mb-4 text-on-surface-variant/30" />
          <h2 className="text-lg font-bold text-on-surface font-[family-name:var(--font-heading)]">Belum Login</h2>
          <p className="text-sm text-on-surface-variant mt-2 mb-6">Masuk untuk melihat profil dan riwayat transaksi Anda.</p>
          <Link href="/login" className="inline-flex items-center gap-2 px-8 py-3 rounded-full gradient-primary text-white font-semibold text-sm hover:opacity-90 transition-opacity shadow-soft">
            Masuk Sekarang
          </Link>
        </div>
      </div>
